...
function ModuleBackButton({ companyId, departmentId, className }: ModuleBackButtonProps) {
  return (
    <Link params={{ companyId, departmentId }} to="/$companyId/$departmentId">
      <Button className={cn("w-full justify-start", className)} size="sm" variant="ghost">
...
function ModuleTitle({ className, ...props }: React.ComponentProps<"h3">) {
  return (
    <h3 className={cn("mb-2 font-semibold text-gray-900 text-sm", className)} data-slot="module-title" {...props} />
  );
}

export const Module = {
  Navigation: ModuleNavigation,
  BackButton: ModuleBackButton,
  Title: ModuleTitle,
};
